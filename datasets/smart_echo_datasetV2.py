...
class KWS:
    ...
    class_dict = {"FULL_LEAK": 0, "MEDIUM_LEAK": 1, "NORMAL": 2}
    ...
    def __ini__(
        ...
        # (data_in_all, data_class_all, data_type_all)
    ...
    def tempRaw_folder(self):
        """Folder for the unsegment raw audio data."""
    ...
    def _custom_dataset(self):
        # convert the LibriSpeech audio files to 1-sec 16KHz .wav, stored under raw

        if self.__check_exists():
            return
        # Need to put the initial data to temp_raw
    ...
    def __filter_classes(self):
        initial_new_class_label = len(self.class_dict)  # 3
        new_class_label = initial_new_class_label  # 3
        for c in self.classes:  # (FUll_LEAK,MEDIUM_LEAK,NORMAL)
            if c not in self.class_dict:
                print(f"Class {c} not found in data")
                return
            num_elems = (self.targets == self.class_dict[c]).cpu().sum()  # targets = labels
            print(f"Class {c} (# {self.class_dict[c]}): {num_elems} elements")
            self.targets[
                (self.targets == self.class_dict[c])
            ] = new_class_label  # convert character to integer label
            new_class_label += 1
...
def SE2_get_datasets(
    data, load_train=True, load_test=True, num_classes=3
...
datasets = [
    {
        "name": "SE2",  # 6 keywords
        "input": (128, 128),
        "output": ("FULL_LEAK", "MEDIUM_LEAK", "NORMAL"),
        "weight": (1, 1, 1),
        "loader": SE2_get_datasets,
    }
]
